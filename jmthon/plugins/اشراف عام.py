...
async def gben(jmthon):
    dc = razan = jmthon
    i = 0
    await dc.get_sender()
    me = await jmthon.client.get_me()
    await razan.edit("▾∮ يتم رفع المستخدم في جميع المجموعات")
    my_mention = "[{}](tg://user?id={})".format(me.first_name, me.id)
    f"@{me.username}" if me.username else my_mention
    await jmthon.get_chat()
    if jmthon.is_private:
        user = jmthon.chat
        rank = jmthon.pattern_match.group(1)
    else:
        jmthon.chat.title
    try:
        user, rank = await get_full_user(jmthon)
    except:
        pass
    if me == user:
    ...
    if user:
        telchanel = [
            d.entity.id
            for d in await jmthon.client.get_dialogs()
        ...
        for x in telchanel:
            try:
                await jmthon.client(EditAdminRequest(x, user, rgt, rank))
                i += 1
