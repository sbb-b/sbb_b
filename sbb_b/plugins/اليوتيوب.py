    ...
    for url in urls:
        ...
        for _path in glob.glob(os.path.join(Config.TEMP_DIR, str(startTime), "*")):
            if _path.lower().endswith((".jpg", ".png", ".webp")):
                ...
                _fpath = _path
        if not _fpath:
            return await edit_delete(sbb_bevent, "**❃ غير قادر على الرفع**")
...
async def download_video(event):
    ...
    reply_to_id = await reply_id(event)
    for url in urls:
        ytdl_data = await ytdl_down(sbb_bevent, video_opts, url)
        if ytdl_down is None:
            return
        try:
            f = pathlib.Path("sbb_b_ytv.mp4")
            print(f)
            sbb_bthumb = pathlib.Path("sbb_b_ytv.jpg")
            if not os.path.exists(sbb_bthumb):
                sbb_bthumb = pathlib.Path("sbb_b_ytv.webp")
            if not os.path.exists(sbb_bthumb):
                sbb_bthumb = None
            await sbb_bevent.edit(
                f"**⌔∮ جار رفع الفيديو :**\
...
async def insta_dl(event):
    ...
    v2 = "@videomaniacbot"
    media_list = []
    sbb_bevent = await edit_or_reply(event, "**⪼ جار التحميل انتظر قليلا**")
    async with event.client.conversation(v1) as conv:
        try:
            v1_flag = await conv.send_message("/start")
        except YouBlockedUserError:
            await edit_or_reply(
                sbb_bevent, "**⪼ لقد حدث خطأ يرجى الغاء الحظر و المحاولة مره ثانيه**"
            )
        ...
        await edit_or_reply(sbb_bevent, "**⪼ جار التحميل من مصدر اخر**")
