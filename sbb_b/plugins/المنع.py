...
async def on_new_message(event):
    ...
    sbb_badmin = await is_admin(event.client, event.chat_id, event.client.uid)
    if not sbb_badmin:
        return
    for snip in snips:
...
@sbb_b.ar_cmd(pattern="منع(?:\s|$)([\s\S]*)")
async def _(event):
...
@sbb_b.ar_cmd(pattern="الغاء منع(?:\s|$)([\s\S]*)")
async def _(event):
    text = event.pattern_match.group(1)
    to_unblacklist = list(
        {trigger.strip() for trigger in text.split("\n") if trigger.strip()}
...
@sbb_b.ar_cmd(pattern="قائمة المنع$")
async def _(event):
